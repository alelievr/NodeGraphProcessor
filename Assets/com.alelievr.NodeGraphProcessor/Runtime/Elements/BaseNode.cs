...
namespace GraphProcessor
{
	...
	public abstract class BaseNode
	{
		...
		/// <summary>
		/// Reorder all edges to the ports generated by a custom behavior so there are no gaps between connected ports
		/// </summary>
		...
		//     
		//     O            ---->O
		public void ReorderEdges(string fieldName)
		{
			Debug.Log("ReorderEdges !");
			// Gather all edges connected to these fields:
			var portDataList = GetPortDataList(fieldName).ToList();
			...
			if (needsFillGap)
			{
				...
				foreach (var edge in lastPortView.GetEdges().ToList())
				{
					...
					graph.Disconnect(edge, false);
					graph.Connect(freePort, edge.outputPort);
				}
			}
		}
		...
		IEnumerable<PortData> GetPortDataList(string fieldName)
		{
		...
		public void UpdatePortsForField(string fieldName)
		{
			...
			foreach (var portData in GetPortDataList(fieldName))
			{
		...
		public bool IsFieldReorderable(string fieldName) => nodeFields[fieldName].allowReordering;
